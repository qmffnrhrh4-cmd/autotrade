...
class Complete20TypeCollector:
    ...
    def wait_for_api_limit(self):
        ...
        if elapsed < 1.0:
            ...
            if self.request_count >= 5:
                wait_time = 1.0 - elapsed + 0.2  # 여유 0.2초
    ...
    def request_tr(self, rqname, trcode, inputs, timeout=10, retry=1):
        """TR 요청 및 대기 (재시도 포함)"""
        for attempt in range(retry + 1):
            if attempt > 0:
                print(f"         🔄 재시도 {attempt}/{retry}")
                time.sleep(1)
    ...
    def collect(self, stock_code):
        ...
        # TR 요청 목록 (19가지)
        tr_list = [
            {'num': 2, 'name': '주식기본정보', 'trcode': 'opt10001', 'inputs': {'종목코드': stock_code}},
            {'num': 3, 'name': '호가잔량', 'trcode': 'opt10004', 'inputs': {'종목코드': stock_code}},
            {'num': 4, 'name': '일봉차트', 'trcode': 'opt10081', 'inputs': {'종목코드': stock_code, '기준일자': target_date, '수정주가구분': '1'}},
            {'num': 5, 'name': '분봉차트', 'trcode': 'opt10080', 'inputs': {'종목코드': stock_code, '틱범위': '1', '수정주가구분': '1'}},
            {'num': 6, 'name': '주식거래량', 'trcode': 'opt10002', 'inputs': {'종목코드': stock_code}},
            {'num': 7, 'name': '체결정보', 'trcode': 'opt10003', 'inputs': {'종목코드': stock_code}},
            {'num': 8, 'name': '시세표성정보', 'trcode': 'opt10007', 'inputs': {'종목코드': stock_code}},
            {'num': 9, 'name': '전일대비등락률', 'trcode': 'opt10005', 'inputs': {'종목코드': stock_code, '기준일자': target_date}},
            {'num': 10, 'name': '투자자별매매동향', 'trcode': 'opt10059', 'inputs': {'일자': target_date, '종목코드': stock_code, '금액수량구분': '1', '매매구분': '0', '단위구분': '1'}},
            {'num': 11, 'name': '종목별투자자기관', 'trcode': 'opt10060', 'inputs': {'종목코드': stock_code, '일자': target_date}},
            {'num': 12, 'name': '외인기관종목별매매', 'trcode': 'opt10061', 'inputs': {'종목코드': stock_code, '기준일자': target_date}},
            {'num': 13, 'name': '프로그램매매종목별', 'trcode': 'opt10062', 'inputs': {'종목코드': stock_code, '시간구분': '0'}},
            {'num': 14, 'name': '시간대별체결가', 'trcode': 'opt10016', 'inputs': {'종목코드': stock_code, '시간구분': '1'}},
            {'num': 15, 'name': '일자별매매상위', 'trcode': 'opt10063', 'inputs': {'종목코드': stock_code, '조회구분': '1'}},
            {'num': 16, 'name': '월별투자자매매', 'trcode': 'opt10064', 'inputs': {'종목코드': stock_code, '시작일자': target_date, '끝일자': datetime.now().strftime('%Y%m%d')}},
            {'num': 17, 'name': '신용잔고', 'trcode': 'opt10013', 'inputs': {'종목코드': stock_code, '기준일자': target_date}},
            {'num': 18, 'name': '시간대별체결조회', 'trcode': 'opt10016', 'inputs': {'종목코드': stock_code, '시간구분': '0'}},
            {'num': 19, 'name': '외인기관순매매', 'trcode': 'opt10061', 'inputs': {'종목코드': stock_code, '기준일자': target_date, '조회구분': '1'}},
            {'num': 20, 'name': '일별체결정보', 'trcode': 'opt10003', 'inputs': {'종목코드': stock_code, '틱범위': '1'}},
        ]
        ...
        if all_data['errors']:
            print(f"\n⚠️  오류 목록:")
            for error in all_data['errors'][:5]:
                print(f"   - {error}")
            if len(all_data['errors']) > 5:
                print(f"   ... 외 {len(all_data['errors']) - 5}개")

        save_json(all_data, f'stock_complete20_{stock_code}')
        return all_data
...
def main():
    """메인 함수"""
    print("=" * 80)
    print("  종목별 20가지 데이터 완전 수집")
    print("  - API 제한 철저 준수 (초당 5회)")
    print("  - 재시도 로직 (2회)")
    print("  - 타임아웃 15초")
    print("  - TR 간 대기 1초")
    print("=" * 80)
    ...
    def on_login(err_code):
        if err_code == 0:
            ...
            def start():
                try:
                    collector = Complete20TypeCollector(api)
                    ...
                    for r in results:
                        ...
                        print(f"   {r['stock_code']} ({r['stock_name']}): {count}가지 수집")
                        if errors > 0:
                            print(f"      ⚠️  {errors}개 오류")

                    save_json({'stocks': results}, 'summary_complete20')
